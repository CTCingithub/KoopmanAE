...
if __name__ == "__main__":
    ...
    train_dl = DataLoader(
        differential_dataset(X_train_recon, horizon), batch_size=batch_size
    )
    test_dl = DataLoader(
        differential_dataset(X_test_recon, horizon), batch_size=batch_size
    )
    ...
    with torch.inference_mode():
        ...
        x_ahead_hat = (
            model.recover(
                model.koopman_operator(
                    model.embed(X_test_recon[[n], [-1], :].to(device).unsqueeze(0)),
                    horizon,
                )
            )
            .cpu()
        ...
        ax.plot3D(
            X_test[n, :, 0], X_test[n, :, 1], X_test[n, :, 2], "k-"
        )  # c=np.linspace(0,1,Time_Length))
