...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch + 1, epochs):
        ...
        for x in tqdm(train_dl):
            optimizer.zero_grad()
            loss_train = koopman_loss(
                x.to(device), model, Sp=Sp, T=T, alpha1=2, alpha2=1e-8
            )
            loss_train.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            for x in tqdm(test_dl):
                loss_test = koopman_loss(
                    x.to(device), model, Sp=Sp, T=T, alpha1=2, alpha2=1e-8
                )
                test_epoch_loss.append(loss_test.cpu().item() * x.shape[0])

            forecast_loss = prediction_loss(
                X_test_recon[:, [-1], :].to(device), X_forecast_test.to(device), model
            )
