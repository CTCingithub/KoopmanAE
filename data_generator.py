...
def load_dataset(dataset_name="lorenz", file_path=r"data", chunk_size=1):
    ...
    with open(
        os.path.join(file_path, f"{dataset_name}/{dataset_name}_train_inputs.pickle"),
        "rb",
    ) as handle:
        train_df = pickle.load(handle)

    with open(
        os.path.join(file_path, f"{dataset_name}/{dataset_name}_test_inputs.pickle"),
        "rb",
    ) as handle:
...
class differential_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        windows = (
            torch.tensor([list(range(i, i + self.horizon)) for i in start])
            .unsqueeze(-1)
            .repeat(1, 1, self.D)
        )
        return torch.gather(self.X[idx], 1, windows).squeeze()
